...
class HomeController extends Controller
{
   ...
   public function __construct()
    ...
    function getsearch (Request $request){
        ...
        $stock_cate = DB::table('stocks')->select('id','name','image','cost','discount','description','category_id')->where('category_id',$id)->where('name','like',"%$search_data%")->orWhere('description','like',"%$search_data%")->get();
        $array = [];
        ...
        $imageStock = array_map("unserialize", array_unique(array_map("serialize", $array)));
        return view("layouts.frontend.partials.search",['stock_cate'=>$stock_cate,'imageStock'=>$imageStock]);
    }
    function search(Request $request){
        ...
        if($request->ajax() && isset($request->start)){
            ...
            $stock_cate = DB::table('stocks')->select('id','name','image','cost','discount','description','category_id')->where('category_id',$id)->where('cost','>=',$start)->where('cost','<=',$end)->get(); // we are fetching all products according to price range
            $array = [];
            foreach($stock_cate as $image){
                $imageStock = DB::table('galleries')->select('id','caption','galleryable_id')->where('galleryable_id',$image->id)->groupBy('galleryable_id')->get();    
                foreach($imageStock as $img){
