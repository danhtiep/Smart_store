...
<div class="clearfix category-product-featured <?php echo $colors[$i].'-box'; ?>" rel="1">
   <div class="category-home-total" style="width: 22%;">
   <?php 
        $fashion_level_1 = DB::table('categories')->where('parent_id',0)->take(1)->get();
    ?>
    @foreach($fashion_level_1 as $item_level_1)
        <?php 
            $fashion_level_2 = DB::table('categories')->where('parent_id',$item_level_1->id)->take(1)->get();
        ?>
        ...
        <div class="category-home-label">
            <a href="#">
                ...
                <span>{!! ucwords($item_level_2->name) !!}</span>
            </a>
        </div>
        @endforeach
        <!-- End Label -->
        ...
        <div class="list-child-category">
            ...
            @foreach($menu_level_3 as $item_level_3)
                <?php 
                    $menu_level_4 = DB::table('categories')->where('parent_id',$item_level_3->id)->take(1)->get();
                ?>
                @foreach($menu_level_4 as $item_level_4)
                <li class="try" value="{!! $item_level_4->id !!}">
                    <span class="pull-right">{!! App\Models\Stock::where('category_id',$item_level_4->id)->count() !!}</span>
                   <a> {!! ucwords($item_level_4->name) !!}</a>
                </li>
                @endforeach
            @endforeach
            </ul>
